...
if ( ! class_exists( 'Location_Grid_Geocoder' ) ) {
    ...
    class Location_Grid_Geocoder {
        ...
        public $geojson;
        public $mapbox_key = 'pk.eyJ1IjoiY2hyaXNjaGFzbSIsImEiOiJjancxMTVjMDYwZHdxNDlwczkxcWtxc2VhIn0.DyM2FWHylzsw4ZlwoaNIZg';
        public $con;
...
//                update_option( 'dt_location_grid_country_levels', $country_levels, false );
        ...
        public function mapbox_get_country_by_coordinates( $longitude, $latitude ) {
            $country_code = false;
            if ( $this->mapbox_key ) {
                $url         = 'https://api.mapbox.com/geocoding/v5/mapbox.places/' . $longitude . ',' . $latitude . '.json?types=country&access_token=' . $this->mapbox_key;

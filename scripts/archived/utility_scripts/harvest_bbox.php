...
foreach( $raw_list as $l ) {
    ...
    if ( $l['level'] !== 'admin2' || $l['level'] !== 'admin3' ) {
        print date('H:i:s') . ' | Skip (' . $l['geonameid'] . ') "Level" not equal to admin2 or admin3' . PHP_EOL;
        continue;
    }
    ...
    $north_latitude = $geojson['bbox'][2];
    $south_latitude = $geojson['bbox'][0];
    $west_longitude = $geojson['bbox'][1];
    $east_longitude = $geojson['bbox'][3];

    $update = mysqli_query( $con, "UPDATE sg_missing_polygons SET

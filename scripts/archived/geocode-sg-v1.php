...
<div id="info_box_container">
    ...
    <p>
        <label><input type="radio" name="level" class="level" value="country"/> Country</label>
        <label><input type="radio" name="level" class="level" value="admin1" checked/> Admin1</label>
        <label><input type="radio" name="level" class="level" value="admin2" /> Admin2</label>
    </p>
    <p>
        <input type="text" id="geoname-input" placeholder="'890098' geonameid" /><br>
        <button type="button" onclick="load_geonameid()">Get Geonames</button>
    </p>
    <p>
...
<script>
        ...
        jQuery.get('https://dt-mapping-builder/get-geonames-lists.php', { type: 'countries', value: 'all' }, null, 'json'  ).done( function(data) {
            if ( data !== undefined ) {
                ...
                jQuery.each( data, function(i,v) {
                    html += '<option value="'+v.admin0_code+'">'+v.name+'</option>'
                })
        ...
        jQuery.get('https://dt-mapping-builder/get-geonames-lists.php', { type: type, value: value }, null, 'json' ).done(function(data) {
            ...
            if ( data !== undefined ) {
                ...
                map.addSource(unique_source, {
                    type: 'geojson',
                    data: 'https://dt-mapping-builder/saturation-grid-project/'+v.level+'/' + v.geonameid + '.geojson'
                });
                map.addLayer({
    ...
    function load_states_by_continent() {
        ...
        jQuery.get('https://dt-mapping-builder/<?php echo basename(__FILE__) ?>', { type: type, value: value }, null, 'json' ).done(function(data) {
            ...
            if ( data !== undefined ) {
                // Info box
                jQuery.each(data, function(i,v) {
                    info_box.append( v.name + ' ('+v.geonameid+')<br>' )
                })
    ...
    map.on('click', function (e) {
        ...
        jQuery.get('https://dt-mapping-builder/location-grid-geocoder.php', { type: 'geocode', longitude: lng, latitude:  lat, level: selected_level }, null, 'json' ).done(function(data) {
            // console.log(data)
            if ( data !== undefined ) {
                let unique_source = '' + data.geonameid + Date.now()
                map.addSource(unique_source, {
                    type: 'geojson',
                    data: 'https://dt-mapping-builder/saturation-grid-project/'+data.level+'/' + data.geonameid + '.geojson'
                });
                map.addLayer({
                    "id": '' + data.geonameid + Date.now() + Math.random(),
                    "type": "fill",
                    "source": unique_source,
                    "paint": {
                        "fill-color": "#888888",
    ...
    geoCode.on('result', function(e) { // respond to search
        ...
        jQuery.get('https://dt-mapping-builder/<?php echo basename(__FILE__) ?>', { type: 'geonameid', longitude: lng, latitude: lat }, null, 'json' ).done(function(data) {
            ...
            jQuery.each( data, function(i,v) {
                ...
                map.addSource(unique_source, {
                    type: 'geojson',
                    data: 'https://dt-mapping-builder/saturation-grid-project/'+v.level+'/' + v + '.geojson'
                });
                map.addLayer({
    ...
    userGeocode.on('geolocate', function(e) { // respond to search
        ...
        jQuery.get('https://dt-mapping-builder/<?php echo basename(__FILE__) ?>', { type: 'geonameid', longitude: lng, latitude:  lat }, null, 'json' ).done(function(data) {
            ...
            jQuery.each( data, function(i,v) {
                ...
                map.addSource(unique_source, {
                    type: 'geojson',
                    data: 'https://dt-mapping-builder/saturation-grid-project/'+data.level+'/' + v + '.geojson'
                });
                map.addLayer({
                    "id": v + Date.now() + Math.random(),
                    "type": "fill",

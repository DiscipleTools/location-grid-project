...
if ( isset( $argv[2] ) && is_numeric( $argv[1] ) ) {
    $target_directory = getcwd() . '/output/' . $argv[1] . '/';
    $new_directory = getcwd() . '/output/simplified_output/' . $argv[1] . '/';
} else {
...
foreach( $files as $file ) {
    if ( array_search( $file, $current_files ) === false ) {
        shell_exec('mapshaper '. $target_directory . $file .' -simplify dp keep-shapes '.$argv[2].'% -o '.$new_directory . $file.' -clean allow-empty');

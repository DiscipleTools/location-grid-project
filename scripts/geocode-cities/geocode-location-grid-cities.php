...
$results = mysqli_query( $script_con, "SELECT * FROM {$table} WHERE admin0_grid_id IS NULL");
...
foreach ($query as $index => $row ) {
    ...
    $full_name = $grid_row['name'];
    if ( isset( $drill_down['admin0_name'] ) ) {
        $admin0_name = $drill_down['admin0_name'] ?? '';
        $admin1_name = $drill_down['admin1_name'] ?? '';

        if ( $admin0_name ) {
            $full_name = $row['name'] . ', ' . $admin0_name;
        }
        if ( $admin1_name ) {
            $full_name = $row['name'] . ', ' . $admin1_name . ', ' . $admin0_name;
        }
    }

    mysqli_query( $script_con, "UPDATE {$table} SET `admin0_name` = '{$drill_down['admin0_name']}' WHERE `id` = {$row['id']};");
    mysqli_query( $script_con, "UPDATE {$table} SET `full_name` = '{$full_name}' WHERE `id` = {$row['id']};");

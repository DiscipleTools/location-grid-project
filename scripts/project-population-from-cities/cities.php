...
SELECT
			...
			# Total Records (44395)
) as tb WHERE population = 0
                " );
if ( empty( $empty_raw ) ) {
    print_r( $con );
    die();
}
$empty = mysqli_fetch_all( $empty_raw, MYSQLI_ASSOC );
...
foreach( $empty as $row ) {
    ...
    $total = 0;
    $city_raw = mysqli_query( $con,
        "SELECT *
                FROM geonames_cities_500 
                WHERE 
...
//        print '.';
    ...
    $cities = mysqli_fetch_all( $city_raw, MYSQLI_ASSOC );

    foreach( $cities as $city ) {
        $total = $total + (int) $city['population'];
    }
    ...
    print $grid_id . ' has ' . $total . PHP_EOL;
//    print "UPDATE location_grid SET population = $total WHERE grid_id = $grid_id;". PHP_EOL;

//     $update = mysqli_query( $con,
//        "UPDATE location_grid SET population = $total WHERE grid_id = $grid_id;" );
//     print_r( $update );
}
